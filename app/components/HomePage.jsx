...
export default function HomePage({ setCurrentPage, lang, allProjects }) {
  ...
  return (
    ...
    <div className="min-h-screen bg-white text-black selection:bg-orange-100">
      ...
      >
        <div className="absolute top-1/4 right-1/4 w-96 h-96 bg-gradient-to-br from-orange-100 to-red-50 rounded-full blur-3xl pointer-events-none" />
        ...
        <motion.h1 className="text-[16vw] md:text-[12vw] lg:text-[10vw] font-bold leading-[0.85] tracking-tighter">
          Portfolio <br />
        ...
        <motion.p className="opacity-60 text-lg md:text-xl max-w-2xl mt-8 font-medium">
          {t.heroSub}
        </motion.p>
        ...
        <motion.div className="flex gap-4 mt-12">
          <button
            ...
            className="px-8 py-4 rounded-full font-bold text-sm uppercase tracking-wider flex items-center gap-2 bg-black text-white shadow-xl shadow-black/20 hover:scale-105 active:scale-95 transition-all"
          >
            {t.heroBtn1}
          ...
          <button
            ...
            className="px-8 py-4 border-2 border-black text-black rounded-full font-bold text-sm uppercase tracking-wider hover:bg-black hover:text-white hover:scale-105 transition-all"
          >
            {t.heroBtn2}
          </button>
        </motion.div>
        ...
        <div className="relative">
          <div className="flex gap-6 overflow-x-auto pb-8 snap-x snap-mandatory scrollbar-hide">
            {featuredProjects.map((project, index) => {
              ...
              return (
                <motion.div key={project.id} className="min-w-[85vw] md:min-w-[500px] snap-center">
                  <div
                    onClick={() => window.location.href = `/projects/${project.slug}`}
                    className="group relative rounded-[2.5rem] border border-black/5 bg-[#f9f9f9] p-10 md:p-12 h-[450px] flex flex-col justify-between cursor-pointer overflow-hidden transition-all duration-500 hover:shadow-2xl hover:-translate-y-2"
                  >
                    <div className="absolute -bottom-4 -right-4 text-[10rem] font-black opacity-[0.03] group-hover:opacity-[0.08] transition-opacity pointer-events-none">
                      {project.year}
                    </div>
                    <div className="flex justify-between items-start z-10">
                      <span className="text-[10px] font-bold uppercase opacity-30">0{index + 1}</span>
                      <div className="w-12 h-12 rounded-full bg-black text-white flex items-center justify-center group-hover:bg-orange-600 transition-colors">
                    ...
                    <div className="z-10">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {(Array.isArray(project.tags) ? project.tags : [])?.slice(0, 3).map((tag) => (
                          <span key={tag} className="px-3 py-1 rounded-full text-[10px] font-bold border border-black/10 bg-white/50">
                            {tag}
                          </span>
                        ))}
                      </div>
